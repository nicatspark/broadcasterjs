...
var eventBus = function () {
  ...
  var on = function (_a) {
    var type = _a[0],
      listener = _a[1],
      _b = _a[2],
      settings = _b === void 0 ? defaultSettings : _b
    var options = setOptions(settings)
    var _c = handleCache().listenerExists(type, listener, options),
      exists = _c.exists,
      id = _c.id
  ...
  var emit = function (type, detail, settings) {
    ...
    return eventTarget.dispatchEvent(
      new CustomEvent('broadcast-' + type, { detail: detail })
    )
  }
  ...
  function handleCache() {
    ...
    var remove = function (type, listener) {
      ...
      broadcastItemsCache = broadcastItemsCache.filter(function (id) {
        return id !== removeId
      })
    }
    ...
    switch (typeof details) {
      ...
      default:
        try {
          ...
          throw new Error(
            'Could not "JSON.stringify" the broadcasterjs payload of "'.concat(
              typeof details,
              '" type.'
            )
          )
